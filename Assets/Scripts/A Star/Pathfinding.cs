...
{
    public AStarGrid GridReference; //For referencing the grid class
    public Transform StartPosition; //Starting position to pathfind from
    public Transform TargetPosition; //Starting position to pathfind to

    // Variables for displaying on comparison scene
    ...
    private void FixedUpdate()
    {
        if (shouldFollow)
        {
            ...
            agent.transform.localPosition = Vector3.MoveTowards(agent.transform.localPosition, CurrentPositionHolder, 35f * Time.fixedDeltaTime);

            if (agent.transform.localPosition == CurrentPositionHolder)
            {
    ...
    // Used for demo scene to toggle movement
