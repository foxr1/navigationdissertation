...
public class AStarGrid : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(transform.localPosition, new Vector3(vGridWorldSize.x, 1, vGridWorldSize.y)); //Draw a wire cube with the given dimensions from the Unity inspector
    ...
    // Finds a suitable location for the goal to be positioned, so that it is not obstruced by buildings and creates an unsolvable path
    public void RandomiseTargetPos()
    {
        ...
        while (!valid)
        {
            randomIndexX = Random.Range(0, iGridSizeX);
            randomIndexY = Random.Range(0, iGridSizeY);

            if (NodeArray[randomIndexX, randomIndexY].bIsWall)
            {
                valid = true;
            }
            else
        ...
        target.transform.localPosition = NodeArray[randomIndexX, randomIndexY].vPosition;
    }
}